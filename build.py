...
EDITOR_TOKEN = ""   # 零长名 token = editor -> e3b0c442….dll
...
# 新增运算符插件：文件名 -> token
OPS = {'op_plus': '+', 'op_minus': '-', 'op_star': '*', 'op_div': '/', 'op_lt': '<', 'op_gt': '>', 'op_le': '<=', 'op_ge': '>=', 'op_eq': '==', 'op_ne': '!=', 'op_and': '&&', 'op_or': '||', 'op_not': '!', 'op_amp': '&', 'op_ior': '|=', 'op_inc': '++', 'op_dec': '--', 'op_iadd': '+=', 'op_qmark': '?', 'op_colon': ':', 'op_assign': '=', 'op_lbracket': '[', 'op_rbracket': ']', 'op_arrow': '->', 'op_dot': '.'}
...
def main():
    ...
    print("[2/7] block.dll（执行器 + sha256 + 网络 + vmstate）")
    sh([GCC, "-shared", "-O2", "-I.", "block.c", "vmstate.c", "net.c", "sha256.c",
        "-o", "block.dll", "-Wl,--export-all-symbols,--out-implib,libblock.dll.a", "-lws2_32"])

    print("[3/7] editor 插件（token = 空名）")
    sh([GCC, "-shared", "-O2", "-x", "c", "-I.", ".c", "-o", dll_of(EDITOR_TOKEN),
        f"-I{RL}/include", f"-L{RL}/lib",
    ...
    for f in sorted(os.listdir(os.path.join(ROOT, "plugins"))):
        ...
        if stem in RAYLIB_FILES:
            args += [f"-I{RL}/include", f"-L{RL}/lib",
                     "-lraylib", "-lopengl32", "-lgdi32", "-lwinmm"]
