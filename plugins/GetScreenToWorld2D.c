/* token="GetScreenToWorld2D" -> sha256 -> c33d30d085dd9a44a8dd9558f17317068b6edcc0ac9dce6699eeeeda33e8fffd.dll  （参考 add.c 的插件结构） */
#include <stdint.h>
#include <stddef.h>
...
__declspec(dllexport) void run(void) {
    ...
    Camera2D cam; Vector2 v;
    *B->stk_off -= 24; memcpy(&cam, B->stk + *B->stk_off, 24);   /* Camera2D = offset+target+rotation+zoom */
    *B->stk_off -= 8;  memcpy(&v,   B->stk + *B->stk_off, 8);
    Vector2 r = GetScreenToWorld2D(v, cam);
    push_bytes(B, &r, 8);
    B->run_next();
