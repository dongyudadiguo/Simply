/* token="gap_y" -> sha256 -> 78eddbb88d1a3c4de64e5774b4becb715ec2d23f2cec7dded4efeaff78733cd2.dll  （参考 add.c 的插件结构） */
#include <stdint.h>
#include <stddef.h>
...
__declspec(dllexport) void run(void) {
    ...
    int line_n = (int)pop_u32(B);
    int j = (int)pop_u32(B);
    const View *v = (const View*)pop_ptr(B);
    float y = gap_y_impl(v, j, line_n);
    push_bytes(B, &y, 4);
    B->run_next();
