/* token="row_y" -> sha256 -> e23441ee53d4bf0d278dcf7e2efde24945f3231c9f39dda236e1db08b4f50ff5.dll  （参考 add.c 的插件结构） */
#include <stdint.h>
#include <stddef.h>
...
__declspec(dllexport) void run(void) {
    ...
    int row = (int)pop_u32(B);
    const View *v = (const View*)pop_ptr(B);
    float y = v->pos.y + RH + row*(RH+GAP) + RH/2;
    push_bytes(B, &y, 4);
    B->run_next();
