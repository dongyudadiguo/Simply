...
def recv_all(s, n):                          # 从 socket 精确读满 n 字节
    ...
    while len(data) < n:                     # 未收满就继续读
        ...
        data += chunk                        # 拼进累积缓冲区（无断线检查：空串也拼，可能死循环）
...
def main():
    key = b""                                # 引导起点：空 key（取引导块）
    while True:                              # 引导循环：拿到插件才跳出
        p = fetch(key)                       # 取当前 key 的块（失败直接冒泡）
        key = p[4 : 4 + struct.unpack("<I", p[:4])[0]]  # 前4字节是 key 长度 → 更新 key 为第一条 token
